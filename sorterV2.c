...
pthread_mutex_t alloc = PTHREAD_MUTEX_INITIALIZER;
...
void* thread_func(void * ptr){
   ...
   // printf("file is: %s\n dir is: %s\n sort by is: %s\n",na->file, na->dir, na->sort);
    ...
    sort_file(input_file,output_file,sort);
...
int main(int argc, char * argv[]){
	...
	**/
	...
	find_csv_files(initial_dir_name, "\t", NULL);
...
int find_csv_files(char *directory_name, char tab[], void * ptr){
        ...
        /**DIRorFILE is not its own file. (dot) .**/
		  ...
		  /**writing in the heiarchy file: extra_credit part:2**/
		    ...
		    strcat(tab_next, tab);
                    ...
                    find_csv_files(path, tab_next, NULL);
...
int sort_file(char input_file_name[],char output_file_name[],char sort[]){
    ...
    printf("lock is acquired\n");
     ...
     strcpy(sort_by, sort);
      ...
      int track_tokenizer=0, null_values=0, non_null_values=0;
...
printf("lock is released at the end\n\n\n");
